...
use crate::service::manager::multi_service_manager::{
...
pub trait SingleServiceManager<T>: ServiceManager<T> {
    ...
    fn get_holder_by_id(&self, id: u16) -> Option<&Box<dyn ServiceHolder<T>>>;
}
...
impl<T> SingleServiceManager<T> for SingleServiceManagerImpl<T> {
    fn get_service_by_id(&self, id: u16) -> Option<&T> {
        self.base.get_services_by_id(id).iter().map(|e| *e).next()
    }

    fn get_holder_by_id(&self, id: u16) -> Option<&Box<dyn ServiceHolder<T>>> {
        self.base.get_holders_by_id(id).iter().map(|e| *e).next()
    }
}
